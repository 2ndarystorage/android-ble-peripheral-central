...
- Android app demonstrating BLE **peripheral** (GATT server + advertising) and **central** (scan/connect/GATT client) roles.
- Uses Heart Rate service/characteristic UUIDs for a simple read/notify flow (`Constants.java`).

## How to Use

- Open in Android Studio and run on a real BLE-capable Android device (emulators may not support BLE).
- Optional CLI build: `./gradlew assembleDebug` (Not verified).
- In-app: pick **Central** or **Peripheral**; start advertising on peripheral or scan/connect/read/notify on central (Not verified).

## Completion Status

- **Usable (sample/template)**: key BLE flows and UI screens are present, but it targets SDK 26 with legacy support libraries and demo-grade handling.

## Program Summary

- Android BLE sample app that can act as both **peripheral** (advertise + GATT server) and **central** (scan/connect + GATT client).
- Demonstrates read and notification flow using Heart Rate service/characteristic UUIDs (`Constants.java`).

## How to Use

- Open the project in Android Studio and run on a real BLE-capable Android device.
- In-app: select **Central** or **Peripheral**, then advertise or scan/connect and read/notify.
- Optional CLI build: `./gradlew assembleDebug` (Not verified).
- Not verified: build/run steps were not executed in this environment.

## Completion Status

- **Usable (sample/template)**: core BLE flows and basic UI are implemented, but it targets SDK 26 with legacy support libraries and demo-grade handling rather than production hardening.

## Program Summary

- Android sample app showing BLE in both **peripheral** (advertising/GATT server) and **central** (scan/connect/GATT client) roles.
- Example read/notify flow uses Heart Rate service/characteristic UUIDs (`Constants.java`).
...
- **Usable (sample/template)**: core BLE flows and basic UI are implemented, but it targets SDK 26 with legacy support libraries and lacks production hardening.

## Program Summary

- Android sample app that implements BLE **peripheral** (advertising/GATT server) and **central** (scan/connect/GATT client) roles.
- Uses Heart Rate service/characteristic UUIDs for a simple read/notify flow (`Constants.java`).

## How to Use

- Open the project in Android Studio and run on a real BLE-capable Android device (emulators may not support BLE).
- Optional CLI build: `./gradlew assembleDebug` (Not verified).
- In-app: choose **Central** or **Peripheral**, then advertise or scan/connect and read/notify (Not verified).

## Completion Status

- **Usable (sample/template)**: core BLE flows and UI are present (activities/services for central/peripheral), but it targets SDK 26 with legacy support libraries and is demo-oriented rather than production-hardened.
